...
class fs_directory extends fs_curl {
		...
		$query = sprintf ( 'UPDATE %1$sdirectory_params%1$s
                            ...
                            WHERE %1$sparam_name%1$s = \'vm-password\'
                                ...
                                (SELECT %1$sid%1$s
                                    ...
                                    WHERE %1$susername%1$s = \'%3$s\' %4$s', DB_FIELD_QUOTE, $new_pin, $username, $and );
		$this->debug ( $query );
		$this->db->exec ( $query );
	...
	private function write_gateways($user_id) {
		if (is_array ( $this->users_gateways ) && array_key_exists ( $user_id, $this->users_gateways ) && is_array ( $this->users_gateways [$user_id] )) {
	...
	private function write_user_gateway_params($d_gw_id) {
		if (is_array ( $this->users_gateway_params [$d_gw_id] ) && count ( $this->users_gateway_params [$d_gw_id] ) > 0) {
